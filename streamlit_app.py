...
def load_data():
    ...
    df['IsAlone'] = df['FamilySize'].apply(lambda x: 1 if x == 1 else 0)  # 1 for Alone, 0 for Not Alone
    ...
    # Преобразование типов для столбцов 'Pclass', 'SibSp' и 'Parch'
...
st.markdown("Веб-приложение для анализа данных о пассажирах Титаника и предсказания выживаемости.")
...
with tab2:
    ...
    if st.button("🚀 Обучить и оценить модель", use_container_width=True):
        ...
        # Сохраняем модель и encoder в сессии для использования на другой вкладке
        st.session_state['model'] = model
        st.session_state['encoder'] = encoder  # Сохраняем encoder
    ...
    if 'model' not in st.session_state:
        ...
        if st.button("Получить прогноз", use_container_width=True, type="primary"):
            try:
                ...
                # Кодируем пользовательский ввод с использованием сохраненного энкодера
                encoder = st.session_state['encoder']
                user_encoded = encoder.transform(user_input)

                # Убедимся, что порядок столбцов и типы данных совпадают
                user_encoded = user_encoded[X_train_encoded.columns]
                for col in X_train_encoded.columns:
                    user_encoded[col] = user_encoded[col].astype(X_train_encoded[col].dtype)
